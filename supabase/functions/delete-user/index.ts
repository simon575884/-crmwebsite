...
Deno.serve(async (req: Request) => {
  ...
  if (!userId || userId === userData.user.id) return respond({ error: "The main administrator cannot be deleted" }, 400);
  ...
  const { data: target, error: targetError } = await adminClient
    ...
    .eq("id", userId)
    .maybeSingle();
  if (targetError) return respond({ error: targetError.message }, 400);
  if (target?.role !== "employee") return respond({ error: "Employee not found" }, 404);

  const { error } = await adminClient.auth.admin.deleteUser(userId);
  if (error) return respond({ error: error.message }, 400);
  return respond({ success: true, deleted_user_id: userId });
});
